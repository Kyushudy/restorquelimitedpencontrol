...
if p.Results.plotNonSaddleTrajectory
    ...
    left   = [0*ones(yPlotNum, 1),     [-10; 10; -10; 10]];
    right  = [xLim(2)*ones(yPlotNum, 1),     [-10; 10; -10; 10]];
    bottom = [[-pi/4; pi/4; -pi/4; pi/4],                       yLim(1)*ones(xPlotNum, 1)];
    top    = [[-pi/4; pi/4; -pi/4; pi/4],                       yLim(2)*ones(xPlotNum, 1)];
...
for cnt = 1:forward_plotNum
    ...
    if cnt == 5
        fill(xsol(1:97,1), xsol(1:97,2), 'g', "FaceAlpha", 0.1);
        fillout(xsol(1:97,1), xsol(1:97,2), [xLim(1) xLim(2) yLim(1) yLim(2)], 'r', "FaceAlpha", 0.1)
    end
end
...
if p.Results.plotEPs
    if ~isempty(SEP_set)
        scatter(SEP_set(:,1),SEP_set(:,2),15, cline,...
               'LineWidth',1.0,'MarkerFaceColor',cline)
    end
    if ~isempty(UEP_set)
        scatter(UEP_set(:,1),UEP_set(:,2),15, cline,...
               'LineWidth',1.0,'MarkerFaceColor',[1,1,1])
    end
end
